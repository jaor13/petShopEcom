...
<div class="w-full max-w-[85rem] py-10 px-4 sm:px-6 lg:px-8 mx-auto pt-0">
  <section class="overflow-hidden bg-white py-11 font-Afacad dark:bg-gray-800">
    ...
    <div class="lg:pl-10">
        <div class="mb-8">
            <h2 class="max-w-xl mb-6 text-2xl font-bold dark:text-black-400 md:text-4xl">
  ...
  <span style="font-family: sans-serif; color: #374151;">Add to Favorites</span>
        ...
        <div class="flex flex-wrap items-center gap-4">
            <a href="{{ auth()->check() ? '#' : route('login') }}" @if(auth()->check())
                ...
                <span wire:loading.remove class="block" wire:target="addToCart({{ $product->id }}, {{ $quantity }})">Add to cart</span>
                <span wire:loading class="hidden block" wire:target="addToCart({{ $product->id }}, {{ $quantity }})">Adding to
                    cart</span>
            </a>
        </div>
    </div>
