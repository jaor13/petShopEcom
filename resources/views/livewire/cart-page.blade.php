<div class="w-full max-w-[90rem] py-10 px-4 sm:px-6 lg:px-8 mx-auto">
    ...
    <table class="w-full">
        ...
        </thead>  <tr><td colspan="5"><hr></td></tr>
        <tbody>
            ...
            <tr wire:key="{{ $item['product_id'] }}">
                <td class="py-4">
                    <div class="flex items-center">
                        ...
                        <a href="{{ url('/product/' . $item['slug']) }}">
                            <img class="h-16 w-16 ml-6 cursor-pointer"
                ...
                <td class="py-4 text-center">
                    <div class="inline-flex items-center border rounded-sm h-8">
                        <button
                            wire:click="decreaseQty({{ $item['product_id'] }}, '{{ $item['variant_name'] ?? '' }}')"
                        ...
                        <span
                            class="w-8 text-center text-sm font-semibold h-full flex items-center justify-center">
                            {{ $item['quantity'] }}
                        </span>
                        <button
                            wire:click="increaseQty({{ $item['product_id'] }}, '{{ $item['variant_name'] ?? '' }}')"
                    ...
                    <button wire:click="removeItem({{ $item['product_id'] }}, '{{ $item['variant_name'] }}')"
                        ...
                        <div style="display: inline-flex; align-items: center;">
                            ...
                            <div wire:loading class="hidden block flex items-center "
                                wire:target="removeItem({{ $item['product_id'] }}, '{{ $item['variant_name'] }}')">
                                <svg xmlns="http://www.w3.org/2000/svg" class="animate-spin  h-5 w-5 mr-1 text-[#F93535]" fill="none" viewBox="0 0 24 24">
