<div class="container-fluid m-3 rounded-3">
    ...
    <a href="{{ url('product/' . $product->slug) }}">
        <img src="{{ url('storage/' . $product->images[0]) }}" class="card-img-top img-fluid" alt="{{ $product->product_name }}" style="object-fit: cover; height: 230px;">
    </a>
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; padding: 5px; width: 100%;">
        <div>
            <div style="color: red; font-weight: bold; font-size: 1.3em;">₱{{ number_format($product->price, 2) }}</div>
            <div>
                <span style="color: gold; font-size: 1.2em; margin-left: -1px;">★</span> 4.5 | {{ $product->sold_count }} sold
            </div>
    ...
    <!-- Cart Icon Slightly Higher -->
    <div style="position: absolute; bottom: 30px; right: 17px; display: flex; align-items: center; justify-content: center; width: 40px; height: 40px; background-color: #00CED1; border-radius: 10px;">
