<div class="container mt-3 p-0">
    ...
    <ul class="d-flex mb-2 bg-white h-14 p-0 pt-3 p-0 px-4 ">
        <li class="nav-item flex-fill text-center custom-link {{ $status === 'all' ? 'active' : '' }}"
            ...
            style="font-weight: normal; max-width: 154px; font-size: 19px; ">
            All Orders

        </li>
        <li class="nav-item flex-fill text-center 
        custom-link  {{ $status === 'to_ship' ? 'active' : '' }} " wire:click.prevent="filterOrders('to_ship')"
            href="?status=to_ship" style="font-weight: normal; max-width: 154px; font-size: 19px;">
            To Ship
        ...
        <li class="nav-item flex-fill text-center custom-link  {{ $status === 'completed' ? 'active' : '' }}"
            ...
            style="font-weight: normal; max-width: 154px; font-size: 18px;">
            Completed
                ...
                <div class="px-5 py-3 shadow-sm  mb-2 bg-white">
                    <div class="d-flex justify-content-between align-items-center border-bottom mb-3">
                        ...
                        <div>
                            <p class="mb-1 mt-3">
                                <span
                                    class="font-semibold text-[#00DCE3]">{{ ucfirst(str_replace('_', ' ', $order['status'])) }}</span>
                            </p>
                        </div>
                        <div class="">
                            <a href="#" class="btn btn-link text-secondary text-decoration-none mt-3"
                                wire:click.prevent="selectOrder({{ $order['id'] }})">Order Details ></a>
                        </div>
                    </div>
                    ...
                    @foreach ($order['items'] as $item)
                        <div class="d-flex align-items-center mb-2">
                            <div class="border  rounded p-1">
                                <img src="{{ $item['image'] ? url('storage', $item['image']) : '' }}" alt="Product Image"
                                    class="img-thumbnail rounded-lg" style="width: 100px; background-color: #E7FAFF; border: none;">
                            </div>
                    ...
                    <div class="text-end mb-4">
                        ...
                        <div class="d-inline-block">
                            ...
                            @else
                                <p class="mb-0 rounded-3 px-3 py-2"
                                    style="border: 2px solid #00DCE3; color: #00DCE3; background-color:hsl(0, 0.00%, 100.00%); font-weight: bold;">
                                    {{ $order['message'] }}
                                </p>
                            @endif
                        </div>
        ...
        <div class="bg-white rounded-lg shadow-lg w-1/3">
            ...
            <div class="flex justify-end p-4 border-t">
                <button type="button" class="bg-gray-500 text-white px-4 py-2 rounded mr-2"
                    onclick="toggleModal('testModal')">Cancel</button>
                <button type="button" class="bg-blue-500 text-white px-4 py-2 rounded"
                    wire:click="updateOrderStatus(document.getElementById('orderId').value)">Confirm</button>
            </div>
        </div>
    </div>
